...
def video2mp3(file_name):
    ...
    :return:
    """
    outfile_name = file_name.split('.')[0] + u'.mp3'
    cmd = u'ffmpeg -i ' + file_name + u' -f mp3 ' + outfile_name
    print(cmd)
...
def video_add_mp3(file_name, mp3_file):
    ...
    :return:
    """
    outfile_name = file_name.split('.')[0] + u'-f.mp4'
    cmd = u'ffmpeg -i ' + file_name + u' -i ' + mp3_file + u' -strict -2 -f mp4 ' + outfile_name
    print(cmd)
    rc = subprocess.call(cmd, shell=True)
    print(rc)
    return rc

def mask_video(input_video, output_video, mask_path=u'mask.jpg'):
    # 打码图片
    ...
    # 已知人脸
    known_image = face_recognition.load_image_file(u"tz.jpg")
...
if __name__ == '__main__':
    # 将音频保存为cut.mp3
    print('start')
    video2mp3(file_name=u'cut.mp4')
    print('1')
    # 处理视频，自动打码，输出视频为output.mp4
    mask_video(input_video=u'cut.mp4', output_video=u'output.mp4')
    print('2')
    # 为 output.mp4 处理好的视频添加声音
    video_add_mp3(file_name=u'output.mp4', mp3_file=u'cut.mp3')
    print('end')